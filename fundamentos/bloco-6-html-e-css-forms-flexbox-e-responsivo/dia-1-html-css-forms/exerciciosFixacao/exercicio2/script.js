...
function anulaEvento(evt) {
    evt.preventDefault()
}

function anulaEventoTexto(evt) {
    if (evt.key !== 'a') {
        evt.preventDefault()
    }
}
...
HREF_LINK.addEventListener('click', anulaEvento);
...
INPUT_CHECKBOX.addEventListener('click', anulaEvento);
...
INPUT_TEXT.addEventListener('keypress', anulaEventoTexto);